	...
	public class CharacterManager
	{
		...
		public CharacterManager(IDbConnection db)
		{
			...
			var table = new SqlTable("tsCharacter",
									 ...
									 new SqlColumn("modPlayers", MySqlDbType.Text)
				);
			...
			try
			{
				using (var reader = database.QueryReader("SELECT * FROM tsCharacter WHERE Account=@0", acctid))
				{
					if (reader.Read())
					{
						...
						playerData.modPlayers = Utils.Base64ToModPlayer(reader.Get<string>("modPlayers"));
						return playerData;
					}
				}
			}
		...
		public bool InsertPlayerData(TSPlayer player, bool fromCommand = false)
		{
			...
			else
			{
				...
				{
					database.Query(
						"UPDATE tsCharacter SET Health = @0, MaxHealth = @1, Mana = @2, MaxMana = @3, Inventory = @4, spawnX = @6, spawnY = @7, hair = @8, hairDye = @9, hairColor = @10, pantsColor = @11, shirtColor = @12, underShirtColor = @13, shoeColor = @14, hideVisuals = @15, skinColor = @16, eyeColor = @17, questsCompleted = @18, skinVariant = @19, extraSlot = @20, modPlayers = @21 WHERE Account = @5;",
						playerData.health, playerData.maxHealth, playerData.mana, playerData.maxMana, String.Join("~", playerData.inventory), player.Account.ID, player.TPlayer.SpawnX, player.TPlayer.SpawnY, player.TPlayer.hair, player.TPlayer.hairDye, TShock.Utils.EncodeColor(player.TPlayer.hairColor), TShock.Utils.EncodeColor(player.TPlayer.pantsColor), TShock.Utils.EncodeColor(player.TPlayer.shirtColor), TShock.Utils.EncodeColor(player.TPlayer.underShirtColor), TShock.Utils.EncodeColor(player.TPlayer.shoeColor), TShock.Utils.EncodeBoolArray(player.TPlayer.hideVisual), TShock.Utils.EncodeColor(player.TPlayer.skinColor), TShock.Utils.EncodeColor(player.TPlayer.eyeColor), player.TPlayer.anglerQuestsFinished, player.TPlayer.skinVariant, player.TPlayer.extraAccessory ? 1 : 0, Utils.ModPlayerToBase64(player.TPlayer));
					return true;
				}
				catch (Exception ex)
				{
			...
			if (!GetPlayerData(player, player.Account.ID).exists)
			{
				try
				{
					database.Query(
						...
						Utils.ModPlayerToBase64(player.TPlayer));
					return true;
				}
				catch (Exception ex)
				{
